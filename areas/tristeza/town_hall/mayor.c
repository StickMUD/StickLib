...
create_monster()
{
	...
	set_ac(20);
	set_hp(500);
	set_long("He is the mayor of StickMUD.");
	load_travel();
...
travel(int nr)
{
string message,action,function;
int time_to_next;
//	remove_call_out("travel");
	sscanf(route[nr],"%d#%s#%s#%s",time_to_next, action, message, function);
	if (message && message!="")
