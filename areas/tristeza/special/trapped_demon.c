...
"This is the ancient daemon Ba\'alzamon, trapped here ages ago\
 ...
 to behold, he waits, brooding, hating, awaiting the day of his escape.");
   ...
   // moved add_armour() before set_level() so that ac will be greater than 1
   ...
   load_chat(8,({
      ...
      "Ba\'alzamon whispers: I will grind my enemies to dust...yes...my time is coming.",
      "Ba\'alzamon laughs!",
      "Ba\'alzamon's eyes gleam red.",
      "Ba\'alzamon growls.",
      "Ba\'alzamon broods darkly.",
...
{
   ::heart_beat();
   if ( !random(4) && query_condition(C_STUNNED) )
     {
        set_condition(C_STUNNED, 0);
        environment()->tell_here("Ba'alzamon breaks free of his paralysis!");
      }
}
int
attack() 
{
   ...
   if ( !ene=all_inventory(environment())) return 0;
   if (!random(3))
    {
