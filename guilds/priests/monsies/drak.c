...
{
    set_name("Drak");
    set_short("Drak, the Arch Bishop of the Holy Order of Boccob");
    set_id(({ "drak","bishop","arch bishop" }));

    set_long("Arch Bishop Drak towers above you, bathed in the "+
      "guiding light of Boccob. His long red robes are bound with a "+
      "single golden rope. Even though he seems quite old and fragile, "+
      "you sense great power is within his grasp. An old, gray bat "+
    ...
    set_group_message("ask_help_msg",
      "Drak yells: Templars! Help me with this heathen!");
    set_group_message("cant_help_msg",
      "Drak shouts: Forgive me brothes, but my hands are full!");
    set_group_message("can_help_msg",
      "Drak bellows: For the Glory of the Church and Boccob!");
...
ring() {
    ...
    att->tell_me("You are hit by the Drak's Ring of Fire!");
...
ice() {
    ...
    if (!(att = query_attack())) return 0;
    att->tell_me("You are hit by the Drak's Bolt of Ice!");
...
fire() {
    ...
    if (!(att = query_attack())) return 0;
    att->tell_me("You are hit by the Drak's fiery Flamestrike!");
...
extra_aggressive(object ob){
	...
	SERVER->inform(sprintf("Drak reports the heathen \
