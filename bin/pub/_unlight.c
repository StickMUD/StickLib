/*
 * New unlight-command (mainly for torches). Same as 'extinguish'.
 * 08-jun-94 Doomdark,
 */
...
unlight_cmd(string s, object me)
{
    object ob;
    string t;
    ...
    if (!s) return notify_fail("Unlight what?\n"), 0;

    if (!sscanf(s, "the %s", t)) t  = s;
    if (ob = present(t, me)) {
	notify_fail("You can't unlight the "+t+".\n");
