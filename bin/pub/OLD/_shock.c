...
shock_cmd(string str, object me)
{
object ob;

	if (!me && !(me = this_player())) return 0;

	if ((int) me -> query_level() < 10) return notify_fail(
		"You are not yet at level 10!\n"), 0;
	...
	if(!ob || !living(ob)) {
		me -> tell_me("Cast the shock at whom?");
		return 1;
	}
	if (ob == me) {
		me -> tell_me("Cast the shock at yourself???");
		return 1;
	}
...
//No delay? //F 2.8.1995
        call_out("shock2",2,({ob,me,environment(me)}));
        return 1;
}
...
shock2(mixed *arr) {
        if(arr && sizeof(arr)==3 && arr[1] && arr[2])
          if(environment(arr[1])!=arr[2])
            arr[1]->tell_me("You moved. Cancelling shock.");
          else
            arr[1]->shock_object(arr[0]);
}
